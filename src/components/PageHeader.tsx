...
export function PageHeader({ eyebrow, title, intro, scripture, image }: Props) {
  return (
    <header className="relative bg-midnight text-white overflow-hidden">
      ...
      <div
        className="absolute -top-32 -right-32 w-[520px] h-[520px] rounded-full breathe pointer-events-none"
        style={{
          background:
            "radial-gradient(circle, rgba(212,164,55,0.25) 0%, transparent 65%)",
        }}
      />
      <div className="absolute top-1/2 -left-24 w-72 h-72 rounded-full opacity-30 pointer-events-none drift"
        style={{
          background:
      ...
      <div className="relative mx-auto max-w-6xl px-6 py-24 lg:py-32">
        ...
        <h1 className="font-display mt-6 text-5xl md:text-6xl lg:text-7xl leading-[1.02] tracking-tight max-w-4xl">
          {title}
        </h1>
        {intro && (
          <p className="mt-7 max-w-2xl text-lg text-white/85 leading-relaxed">
            {intro}
          </p>
        )}
        {scripture && (
          <figure className="mt-12 max-w-xl border-l-2 border-gold pl-6">
            <blockquote className="font-display italic text-xl text-white/95 leading-snug">
