...
// The currency table itself lives in lib/currencies.ts so that a client
// component can render a price without dragging this gateway client — and its
// secret-key handling — into the browser bundle. Re-exported here so existing
...
async function flwFetch<T>(
  ...
  try {
    ...
    throw new FlutterwaveError(
      timedOut
        ? `Flutterwave did not respond within ${REQUEST_TIMEOUT_MS / 1000}s`
