...
export function CartClient({ available }: { available: CurrencyCode[] }) {
  const {
    items,
    currency,
    ready,
                    ...
                    <div className="mt-3 flex items-center gap-4">
                      <div className="inline-flex items-center border border-midnight/20">
                        <button
                          ...
                          aria-label={`Fewer copies of ${item.title}`}
                        ...
                        <button
                          type="button"
                          onClick={() => setQuantity(item.bookId, item.quantity + 1)}
                          aria-label={`More copies of ${item.title}`}
                          className="px-2.5 py-1.5 text-midnight/70 hover:bg-midnight hover:text-gold transition-colors"
                        >
                          <Plus size={13} />
                        </button>
          ...
          <>
            ...
            >
              {unavailable.length === 1
                ? `“${unavailable[0].title}” is not sold in ${currency}.`
                : `${unavailable.length} books in your basket are not sold in ${currency}.`}{" "}
              Choose another currency or remove{" "}
